...
_TOOLKIT: Dict[str, RetailTools] = {
    "instance": RetailTools(RetailDB.load(RETAIL_DB_PATH))
}


def _get_toolkit() -> RetailTools:
...
def load_scenario(scenario: dict | None = None):
    """Load a database snapshot for the retail toolkit."""
    try:
        if scenario and isinstance(scenario, dict) and "db" in scenario:
            db = RetailDB.model_validate(scenario["db"])
        else:
            db = RetailDB.load(RETAIL_DB_PATH)
        _TOOLKIT["instance"] = RetailTools(db)
        return "Successfully loaded retail scenario."
...
@mcp.tool()
def save_scenario():
    """Serialize the current retail database to a dictionary."""
    toolkit = _get_toolkit()
    if toolkit.db is None:
        return {"error": "Retail database is not initialized."}
