...
_TOOLKIT: Dict[str, MockTools] = {
    "instance": MockTools(MockDB.load(MOCK_DB_PATH))
}


def _get_toolkit() -> MockTools:
...
def load_scenario(scenario: dict | None = None):
    """Load a database snapshot for the mock toolkit."""
    try:
        if scenario and isinstance(scenario, dict) and "db" in scenario:
            db = MockDB.model_validate(scenario["db"])
        else:
            db = MockDB.load(MOCK_DB_PATH)
        _TOOLKIT["instance"] = MockTools(db)
        return "Successfully loaded mock scenario."
...
@mcp.tool()
def save_scenario():
    """Serialize the current mock database to a dictionary."""
    toolkit = _get_toolkit()
    if toolkit.db is None:
        return {"error": "Mock database is not initialized."}
