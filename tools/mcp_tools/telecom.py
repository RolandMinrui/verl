...
_TOOLKIT: Dict[str, TelecomTools] = {
    "instance": TelecomTools(TelecomDB.load(TELECOM_DB_PATH))
}


def _get_toolkit() -> TelecomTools:
...
def load_scenario(scenario: dict | None = None):
    """Load a database snapshot for the telecom toolkit."""
    try:
        if scenario and isinstance(scenario, dict) and "db" in scenario:
            db = TelecomDB.model_validate(scenario["db"])
        else:
            db = TelecomDB.load(TELECOM_DB_PATH)
        _TOOLKIT["instance"] = TelecomTools(db)
        return "Successfully loaded telecom scenario."
...
@mcp.tool()
def save_scenario():
    """Serialize the current telecom database to a dictionary."""
    toolkit = _get_toolkit()
    if toolkit.db is None:
        return {"error": "Telecom database is not initialized."}
