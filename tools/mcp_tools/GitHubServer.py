...
class GitHubServer:
    ...
    def save_scenario(self) -> dict:
        ...
        return {
            ...
            "issues": {k: {str(ik): iv.dict() for ik, iv in v.items()} for k, v in self.issues.items()},
            "pull_requests": {k: {str(ik): iv.dict() for ik, iv in v.items()} for k, v in self.pull_requests.items()},
            "branches": {k: {ik: iv.dict() for ik, iv in v.items()} for k, v in self.branches.items()},
            "commits": {k: [c.dict() for c in v] for k, v in self.commits.items()},
